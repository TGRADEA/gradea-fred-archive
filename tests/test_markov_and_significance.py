...
def test_markov_labels_survive_deleting_the_future(panel):
    ...
    full, _ = markov_regime(panel.pit)
...
def test_belief_is_a_probability_distribution(panel):
    _, beliefs = markov_regime(panel.pit)
    rows = beliefs.dropna()
...
def test_learned_states_are_ordered_by_volatility(panel):
    """Low must actually be calmer than High. Percentile seeding guarantees the
    emission means come out monotone; this checks the guarantee survives the
    filtering step and the relabelling."""
...
def test_known_turmoil_is_classified_as_high_volatility(panel):
...
def test_calibration_refuses_a_window_too_short_to_fit():
    model = MarkovVolatilityRegime()
    before = model.emission_means.copy()
    assert model.calibrate(np.array([0.001, 0.002, 0.003])) is False
    assert np.array_equal(model.emission_means, before)


def test_filter_handles_an_impossible_observation():
    """An observation far outside all three Gaussians makes every likelihood
    underflow to zero. The filter must fall back to its prediction rather than
    divide by zero and emit NaN for the rest of the sample."""
    model = MarkovVolatilityRegime()
    belief = model.step(1e6)
